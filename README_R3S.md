...
**Здесь легко испортить всю схему.** Установщик прописывает зарубежный
резолвер по HTTPS (`https://dns10.quad9.net/dns-query`), а такие обращения
из России идут нестабильно. AGH ждёт таймаута на каждом запросе, и страницы
перестают открываться — при этом ни один фильтр не срабатывает, и по журналу
блокировок причина не видна.

То же касается загрузочных серверов: если `bootstrap_dns` зарубежные, AGH не
сможет разрешить даже имя своего вышестоящего.

Рабочий вариант — резолвер, доступный изнутри страны:
...
`blocking_mode: nxdomain` вместо `default`: по умолчанию AGH отвечает
адресом `0.0.0.0`, и браузер пытается туда подключиться, дожидаясь отказа.
`nxdomain` отвечает «домена нет», и страницы с синхронно подключённой
аналитикой перестают подвисать.

Шифрования при таком выборе нет — запросы идут открытым текстом. Провайдер
видит, какие домены разрешает роутер, но связать их с конкретным устройством
не может. Если нужен DoH, сервер стоит подбирать через кнопку **Тестировать
вышестоящие DNS-серверы** на странице настроек.

Замерить результат:

```bash
time nslookup ya.ru 127.0.0.10     # десятки миллисекунд, не секунды
```
...
nslookup doubleclick.net      # NXDOMAIN — заблокирован фильтром
...
Фильтры регулярно ловят домены, без которых страницы не собираются. Симптом
характерный: страница висит, а в журнале AGH ровно в этот момент появляются
блокировки. Скорость разрешения имён при этом нормальная, и по общим
проверкам причина не видна.

Из встреченного на практике:

| Домен | Что это | Последствие блокировки |
|---|---|---|
| `abt.s3.yandex.net` | A/B-эксперименты Яндекса | Интерфейс ждёт конфигурацию и не отрисовывается |
| `mc.yandex.ru` | счётчик Метрики | Страницы с синхронным скриптом ждут его загрузки |

Исключения добавляются в **Фильтры → Пользовательские правила фильтрации**,
по правилу на строку:
...
Действуют сразу и переживают обновление списков, в отличие от разблокировки
отдельной записи через журнал.

Туда же стоит сразу занести домены своих прокси-серверов:

```
@@||tgtesla.click^
```

Блокировщик рекламы, отрезающий канал обхода блокировок, диагностируется
особенно мучительно — симптом выглядит как отказ сервера.

Проверять исключения с клиента, не забыв сбросить кеш:

```bash
sudo dscacheutil -flushcache; sudo killall -HUP mDNSResponder
nslookup abt.s3.yandex.net     # ждём настоящий адрес, не NXDOMAIN
```
